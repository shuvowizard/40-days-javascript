...
try {
    ...
    console.log(err.stack)  // ReferenceError: abc is not defined at script.js:58:5
}

// Real-World Use Cases
// `new Error()` is a constructor function that creates a new Error object instance. The Error object represents an error that has occurred during the execution of a program.
function divideNumbers(a, b) {
    try {
        if (b === 0) {
            const err = new Error("Division by zero is not allowed."); // (Custom Error)
            throw err;
        }
        const result = a/b;
...
try {
    validateForm({ username: "Shuvo", email: "bademail" })
} catch(error) {
